...
// Real 100XU blueprint: 10 blocks x 10 days. Every day runs the block's full
// V1..V10 circuit: 10 reps x 10 rounds = 1,000 reps/day, 100,000 total.
// Run: npm run db:seed:challenge
// Demo videos: paste URLs later via admin PUT /api/admin/challenges (demoVideoUrl).

type Task = { name: string; load: string; cue: string; format?: string };
type Block = { title: string; focus: string; tasks: Task[] };

const R10 = '10 Reps x 10 Rds';
const RECOVERY =
...
const BLOCKS: Block[] = [
  {
    ...
    tasks: [
      ...
      { name: 'Plyo Box Jumps (Step Down)', load: '20-24" Box', cue: 'Two-foot explosive takeoff, quiet athletic landing, step down to reset.' },
      ...
      { name: 'Box Step-Overs (Facing Box)', load: '20-24" Box', cue: 'Step up onto box center, pivot, and step down facing opposite side smoothly.' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { name: 'Box Jump-Overs', load: '20-24" Box', cue: 'Jump up onto box top, hop down the far side, immediate pivot to repeat.' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { name: 'Lateral Box Step-Overs', load: '20" Box', cue: 'Travel sideways over box; plant lead foot, stand tall, step down other side.' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { name: 'Box Rapid Toe-Taps', load: '20" Box', cue: 'High cadence turnover; spring off balls of feet, alternate tapping box top.', format: '10 Reps (L+R=1) x 10 Rds' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { name: 'Low Sled Drive: Quad Bear Crawls', load: 'Bodyweight', cue: 'Knees hovering 1" off floor; forward crawl maintaining low 45 deg torso angle.', format: '10 Paces x 10 Rds' },
      ...
      { name: 'Plyo Box High Jumps', load: '24" Box', cue: 'Vertical explosion, land softly, stand to complete hip lockout at top.' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { name: 'Box Rebound Jumps', load: '20" Box', cue: 'Land softly on box, step down, instantly spring off forefoot back onto box.' },
      ...
      { name: 'Burpee Box Step-Overs', load: '20-24" Box', cue: 'Burpee facing box, jump up, step across top of box to opposite floor.' },
    ],
  },
  {
    ...
    tasks: [
      ...
      { name: 'High Box Jumps (Step Down)', load: '24-30" Box', cue: 'Maximum vertical hip drive; land softly in quarter squat; step down to reset.' },
      ...
      { name: 'Box Depth Drop to Vertical Pop', load: '20" Box', cue: 'Step off box, hit floor, instantaneously redirect kinetic energy straight up.' },
      ...
      { name: 'Burpee Box-Over Jumps', load: '20-24" Box', cue: 'Chest-to-deck burpee, spring over box top to far side without standing upright.' },
    ],
  },
  {
    ...
    tasks: [
      ...
      { name: 'Plyo Box Jumps with Soft Stick', load: '24" Box', cue: 'Absorb landing silently; stand tall to full vertical hip extension.' },
      ...
      { name: 'Box Lateral Shuffle Taps', load: '20" Box', cue: 'Lateral shuffle tap top of box, drop to side, immediate athletic rebound.' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { name: 'Box Rebound Jumps', load: '20-24" Box', cue: 'Minimal floor contact time; land on box and spring back instantly.' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { name: 'Box Jumps to Stand Tall', load: '24" Box', cue: 'Land softly on top of box, stand to full vertical hip lockout before stepping down.' },
...
async function main() {
  ...
  await db.challengeDay.deleteMany({ where: { challengeId: challenge.id } });
  for (let b = 0; b < BLOCKS.length; b++) {
    const block = BLOCKS[b];
    if (!block) continue;
    for (let d = 1; d <= 10; d++) {
      const dayNumber = b * 10 + d;
  ...
  const days = await db.challengeDay.count({ where: { challengeId: challenge.id } });
  const tasks = await db.challengeTask.count({ where: { day: { challengeId: challenge.id } } });
  console.log(`Seeded 100XU: ${days} days, ${tasks} tasks`);
  await db.$disconnect();
}
